...
A Monte Carlo simulation study was conducted for power estimation in the software Mplus (Muth?n & Muth?n, 2014). An introduction to Monte Carlo power analysis in Mplus is provided in Muthen and Muthen (2002). A discussion of  the specification of intervention effects is given in Thoemmes, MacKinnon, and Reiser (2010).
...
library(ggplot2)
MCdata <- data.frame(Sample_Size=c(500, 600, 700, 800, 900, 1000, 1100, 1200), Power = c(0.66, 0.74, 0.82, 0.86,
                                                                                         0.90, 0.92, 0.94, 0.96))

ggplot(MCdata, aes(x=Sample_Size, y=Power)) +
  geom_point(size = 5, shape=2) +
  geom_line(size = 2, colour = "grey30") +
  scale_x_continuous("Sample Size", breaks=c(500, 600, 700, 800, 900, 1000, 1100, 1200)) +
  scale_y_continuous("Power", breaks = c(.60, .65, .70, .75, .80, .85, .90, .95)) +
  ...
  geom_hline(yintercept=.80, color ="red", size = 1, linetype = 1)
```


##References
Muth?n, B., & Muthen, L. (2002). How to decide on smaple size using Monte Carlo Simulations in Mplus. Structurla Equation Modeling, 2, 599-638.
Muth?n, B., & Muthen, L. (2014). Mplus User's Guide. Brooklyn, NY.
